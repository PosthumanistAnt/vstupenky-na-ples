...
<div
    x-data='Alpine.component("searchableMultiselect")({
        values: $wire.entangle("{{ $attributes->wire('model')->value() }}"),
        data: @json($options),
...
>
    <div class="w-full rounded-md relative">
        <button
            ...
            class="flex items-center z-0 w-full py-2 pl-3 pr-10 text-left transition duration-150 ease-in-out bg-white border border-gray-300 cursor-default rounded-md styled-focus"
        >
            <div
                x-text="values.map(value => options[value]).join(', ') || placeholder"
                :class="{ 'text-gray-400': ! values.length }"
                class="w-full truncate break-all leading-6 text-sm block"
            ></div>
    ...
    <div class="w-full">
        ...
        >
            <input
                ...
                {{-- x-on:blur="blurredInput()" --}}
                type="search"
                :placeholder="placeholder"
                class="w-full block truncate leading-6 text-base border-none form-input focus:outline-none focus:ring-0 focus:ring-offset-0"
            />
            <ul
                x-ref="listbox"
                @keydown.enter.stop.prevent="selectOption()"
                ...
                tabindex="-1"
                class="py-1 text-base overflow-auto scrollbar-transparent leading-6 rounded-md max-h-60 focus:outline-none sm:text-sm sm:leading-5"
            >
